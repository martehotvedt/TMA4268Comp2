...
To solve the prediction of age by using MLR, we first use the $\texttt{lm()}$-function to estimate the regression coefficients $\beta$:
...
First, we need to check that that the model assumptions of a MLR actually hold. To check this we may use the model checking tools Tukey-Anscombe diagram and the QQ-diagram. 

```{r, eval = TRUE, echo = FALSE}
plot(mlr_mod, which = c(1,2))
```
